defmodule RemitWeb.CommentComponent do
  ...
  defp comment_resolution_block(assigns) do
    cond do
      assigns.resolved? ->
        notification_resolved(assigns)

      assigns.comment_resolved? ->
        comment_resolved_by_coauthor(assigns)

      true ->
        not_resolved(assigns)
    end
  end

  defp notification_resolved(assigns) do
    ~H"""
  ...
  defp comment_resolved_by_coauthor(assigns) do
    ~H"""
    <i class="fas fa-check text-green-dark"></i>
    <span class="text-gray-dark"><%= Utils.format_datetime(@notification.comment.resolved_at) %></span>

    <i class="fas fa-user-friends text-blue-600"></i>
    <span class="text-gray-dark">by <%= @notification.comment.resolved_by %></span>
    """
  end

  defp not_resolved(assigns) do
    ~H"""
    <%= if @at_me? do %>
